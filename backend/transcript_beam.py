...
class TranscribeAudioFn(beam.DoFn):
    ...
    def process(self, element):
        ...
        try:
            ...
            audio_uri = normalized_path

            print(f"\n{'='*50}")
            print(f"Transcribing: {audio_name}")
            print(f"{'='*50}")

            # extract language code from audio_name
            # format: uuid_originalname_en-US
            language_code = audio_name.split("_")[-1]
            print(f"  🌐 Language: {language_code}")

            speech_client = speech_v1.SpeechClient()

            # check duration to pick right model and method
            ...
            if not response.results:
                print(f"  ⚠️  No speech detected in {filename}")
                self._update_failed(bq_client, audio_name, "no speech detected")
                return
...
def run():
    print(f"\n{'='*50}")
    print(f"Starting Beam transcription at {datetime.now()}")
    print(f"{'='*50}\n")
    ...
    with beam.Pipeline(options=options) as pipeline:
        (
            ...
            | "Transcribe audio" >> beam.ParDo(TranscribeAudioFn())
            | "Print results" >> beam.Map(lambda x: print(f"✅ Completed: {x}"))
        )

    print(f"\n{'='*50}")
    print(f"🎉 Transcription pipeline complete!")
    print(f"{'='*50}\n")
