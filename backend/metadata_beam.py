...
class ExtractMetadataFn(beam.DoFn):

    def process(self, element):
        audio_name, transcript_path, normalized_path = element
        bq_client = bigquery.Client()
        ...
        try:
            ...
            # download normalized audio for duration
            ...
            # load into BigQuery audio_metadata table
...
def run():
    print(f"\n{'='*50}")
    print(f"Starting Beam metadata extraction at {datetime.now()}")
    print(f"{'='*50}\n")
    ...
    with beam.Pipeline(options=options) as pipeline:
        (
            ...
            | "Extract metadata" >> beam.ParDo(ExtractMetadataFn())
            | "Print results" >> beam.Map(lambda x: print(f"✅ Completed: {x}"))
        )

    print(f"\n{'='*50}")
    print(f"🎉 Metadata pipeline complete!")
    print(f"{'='*50}\n")
